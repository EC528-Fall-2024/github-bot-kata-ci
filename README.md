...
- Implemented (at least in our Next.js/React repo)

  - Converted to Next.js/React
  - Filter for 'Required' tests (fetching data from the main branch)
  - Added PR view to show results of PR checks
  - URL search feature (search via either URL extension parameters or the built-in table filter)
  - Fetch and report rerun jobs (with a tooltip to see the result of each job)
  - Split the Nightly and PR fetch scripts
  - Simple bar chart for summary of
  - Early implementation of maintainer link (link to maintainer for the test)
  - Authenticate GitHub API calls
- Planned

  - Implement graphs view
  - Implement a tree view
  - Update maintainer link dynamically
...
- Implemented
  ...
  - Early draft proposal for Prow implementation
- Planned
